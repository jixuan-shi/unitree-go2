...
def generate_launch_description():
    ...
    footprint_to_odom_ekf = Node(
        ...
        condition=UnlessCondition(LaunchConfiguration("close_loop_odom")),
    )

    footprint_to_odom_gazebo_ekf = Node(
        package="robot_localization",
        executable="ekf_node",
        name="footprint_to_odom_ekf",
        output="screen",
        parameters=[
            ...
            os.path.join(
                ...
                "config",
                "ekf",
                "footprint_to_odom_gazebo.yaml",
            ),
        ],
        remappings=[("odometry/filtered", "odom")],
        condition=IfCondition(LaunchConfiguration("close_loop_odom")),
    )
    ...
    base_link_to_footprint_tf = Node(
        package="tf2_ros",
        executable="static_transform_publisher",
        name="base_link_to_footprint",
        arguments=["0", "0", "-0.275", "0", "0", "0", "base_link", "base_footprint"],
        parameters=[{"use_sim_time": LaunchConfiguration("use_sim_time")}],
    )
    ...
    return LaunchDescription(
        [
            ...
            footprint_to_odom_gazebo_ekf,
            world_to_odom_tf,
            base_link_to_footprint_tf,
            rviz2
        ]
    )
